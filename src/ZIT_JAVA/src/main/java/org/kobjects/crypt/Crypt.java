...
public class Crypt {
    ...
    private static int[] des_set_key(byte[] key) {
        ...
        PERM_OP(HPERM_OP(results[0], -2, -859045888), HPERM_OP(results[1], -2, -859045888), 1, 1431655765, results);
        ...
        for (int i3 = 0; i3 < 16; i3++) {
            ...
            int j2 = j + 1;
            schedule[j] = ((t << 16) | (65535 & s)) & -1;
            int s2 = (s >>> 16) | (-65536 & t);
            j = j2 + 1;
            schedule[j2] = ((s2 << 4) | (s2 >>> 28)) & -1;
        }
        return schedule;
    }
    ...
    private static final int D_ENCRYPT(int L, int R, int S, int E0, int E1, int[] s) {
        ...
        int t2 = (t >>> 4) | (t << 28);
        return L ^ (((((((SPtrans[1][t2 & 63] | SPtrans[3][(t2 >>> 8) & 63]) | SPtrans[5][(t2 >>> 16) & 63]) | SPtrans[7][(t2 >>> 24) & 63]) | SPtrans[0][u2 & 63]) | SPtrans[2][(u2 >>> 8) & 63]) | SPtrans[4][(u2 >>> 16) & 63]) | SPtrans[6][(u2 >>> 24) & 63]);
    }
    ...
    private static final int[] body(int[] schedule, int Eswap0, int Eswap1) {
        ...
        int[] results = new int[2];
        PERM_OP(((left >>> 1) | (left << 31)) & -1, ((right >>> 1) | (right << 31)) & -1, 1, 1431655765, results);
        PERM_OP(results[1], results[0], 8, 16711935, results);
    ...
    public static final String crypt(String salt, String original) {
        ...
        int i2 = 0;
        while (i2 < key.length && i2 < original.length()) {
            key[i2] = (byte) (original.charAt(i2) << 1);
            i2++;
        }
        ...
        for (int i3 = 2; i3 < 13; i3++) {
            ...
            while (j < 6) {
                ...
                buffer.setCharAt(i3, (char) cov_2char[c]);
                j++;
                u = u2;
            }
        }
